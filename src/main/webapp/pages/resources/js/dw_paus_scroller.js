...
/*
 * dw_event.js - basic event handling file from dyn-web.com version date May
 * 2013 (added .domReady) .domReady uses whenReady fn from JavaScript the
 * Definitive Guide 6th edition by David Flanagan, example 17.01
 */
...
DYN_WEB.Util = (function(Ut) {
	...
	Ut.mouseleave = function(e, oNode) {
		e = Ev.DOMit(e);
		var toEl = e.relatedTarget ? e.relatedTarget
	...
	var Glide_Scroll = {
		...
		doGlideScroll : function() {
			...
			if (elapsed < this.slideDur) {
				var x = this.startX
						+ (this.distX * Math.sin(this.per * elapsed));
				var y = this.startY
						+ (this.distY * Math.sin(this.per * elapsed));
				this.shiftTo(x, y);
				this.on_scroll(x, y);
			} else {
				this.clearTimer();
				...
				if (this.distX
						&& (this.destX === 0 || this.destX === -this.maxX)
						|| this.distY
						&& (this.destY === 0 || this.destY === -this.maxY)) {
	...
	var Pause_Auto_Scroll = {
		makePauseAuto : function(opts) {
			...
			if (opts.bRepeat && opts.repeatId) {
				var pos = Ut
						.getLayerOffsets(Ut.$(opts.repeatId), Ut.$(this.id));
				if (pos.x || pos.y) {
